...
impl Clade {
	...
	/// Collapses a node with a single child.
	pub fn collapse(&mut self) {
		if self.children.len() != 1 {
			return; // Find a better way to handle this.
		}
		self.name = self.children[0].name.clone();
		self.branch_length += self.children[0].branch_length;
		self.tags = self.children[0].tags.clone();
...
impl Clade {
	...
	pub fn annotate(&mut self, annotations: &HashMap<String, HashMap<String, CladeMeta>>) {
		...
		if !set {
			if annotations.contains_key("##Default##") {
				self.tags.extend(annotations["##Default##"].clone())
			}
		}
	...
	pub fn find_common_ancestor(&self, name_list: &mut Vec<String>) -> Option<&Clade> {
		...
		// Cut down to just the remaining names
		*name_list = trimmable_list;
	...
	/// 
	pub fn trim(&mut self, nodes: &HashMap<String, String>, collapse: bool) {
		...
		if (self.children.len() == 1) && collapse {
			self.collapse();
		}
	}

	/// Remove leaves
	pub fn filter(&mut self, nodes: &Vec<String>, collapse: bool) {
		// store a list of nodes that need to be pruned to avoid pruning while traversing
		// Because HashMaps don't implement equality we can't do this the clean way
		// with &Clade & remove_item
		let mut to_prune: Vec<usize> = Vec::new();
		...
		for clade_pos in to_prune.iter() {
			self.children.remove(*clade_pos);
		}
		
		if (to_prune.len() > 0) && (self.children.len() == 0) {
			// If we've removed all this node's children, set this node to collapse by setting it to a removable name.
			self.name = nodes.first().unwrap().replace('_', " ");
		}

		if (self.children.len() == 1) && collapse {
			// We are collapsing this node since it has only one child.
			self.collapse();
		}
	}
